...
bosList$train <- sample(c(0, 1), nrow(bosList), replace = TRUE, prob = c(.3, .7))
...
bjList$train <- sample(c(0, 1), nrow(bjList), replace = TRUE, prob = c(.3, .7))
...
```{r}
Boston.boost=gbm(formula = price~., distribution = "gaussian", data = boslist_train, n.trees = 800,interaction.depth = 15, shrinkage = 0.005,cv.folds = 5)
...
```{r}
beijing.boost=gbm(formula = price~., distribution = "gaussian", data = bjList_test, n.trees = 800,interaction.depth = 15, shrinkage = 0.005,cv.folds = 5)
